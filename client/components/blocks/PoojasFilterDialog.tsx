...
type PoojasFilterDialogProps = {
  ...
  getBenifitLabel: (benifit: Benifit, language: PoojasBrowserDbLanguage) => string;
  getTempleLabel: (temple: Temple, language: PoojasBrowserDbLanguage) => string;
  onApply: (category: PoojaCategoryFilter, benifitId: string, templeId: string) => void;
...
export function PoojasFilterDialog({
  ...
  const [tempCategory, setTempCategory] = useState<PoojaCategoryFilter>(activeCategory);
  ...
  const filteredBenefits = useMemo(() => {
    ...
    return benifits.filter((b) => getBenifitLabel(b, selectedDbLanguage).toLowerCase().includes(query));
  ...
  const filteredTemples = useMemo(() => {
    ...
    return temples.filter((t) => getTempleLabel(t, selectedDbLanguage).toLowerCase().includes(query));
      ...
      <DialogContent className="flex max-h-[90vh] w-[95vw] max-w-3xl flex-col gap-0 overflow-hidden rounded-2xl p-0 sm:max-h-[700px]">
        <DialogHeader className="border-b border-black/10 px-6 py-4">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-xl font-extrabold text-text-primary">Filters</DialogTitle>
          </div>
        </DialogHeader>
          ...
          <div className="flex flex-shrink-0 flex-row overflow-x-auto border-b border-black/10 bg-[#f9fafb] md:w-[240px] md:flex-col md:border-b-0 md:border-r">
            {TABS.map((tab) => (
              ...
              >
                ...
                {tab.label}
                {tab.id === "pujaType" && tempCategory && <span className="ml-1 text-saffron">*</span>}
                {tab.id === "benefit" && tempBenifitId && <span className="ml-1 text-saffron">*</span>}
                {tab.id === "templeType" && tempTempleId && <span className="ml-1 text-saffron">*</span>}
              </button>
            ))}
          </div>
                ...
                <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-black/10 p-4 transition-colors hover:bg-black/5">
                  <div
                    className={cn(
                      ...
                      tempCategory === "normal" ? "border-saffron bg-saffron" : "border-black/20"
                    )}
                  >
                    {tempCategory === "normal" && <Check className="h-3 w-3 text-white" />}
                  </div>
                  <input
                    type="checkbox"
                    className="sr-only"
                    checked={tempCategory === "normal"}
                    onChange={(e) => setTempCategory(e.target.checked ? "normal" : "")}
                  />
                  <span className="text-sm font-bold text-text-primary">One Day Pooja</span>
                </label>
                <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-black/10 p-4 transition-colors hover:bg-black/5">
                  <div
                    className={cn(
                      "flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2",
                      tempCategory === "weekly" ? "border-saffron bg-saffron" : "border-black/20"
                    )}
                  >
                    {tempCategory === "weekly" && <Check className="h-3 w-3 text-white" />}
                  </div>
                  <input
                    type="checkbox"
                    className="sr-only"
                    checked={tempCategory === "weekly"}
                    onChange={(e) => setTempCategory(e.target.checked ? "weekly" : "")}
                  />
                  <span className="text-sm font-bold text-text-primary">Weekly Pooja</span>
                </label>
              </div>
            )}
                ...
                <div className="overflow-y-auto pr-2 scrollbar-thumb-saffron">
                  <div className="space-y-1">
                    {filteredBenefits.map((b) => (
                      <label key={b.id} className="flex cursor-pointer items-center gap-4 rounded-md p-2 transition-colors hover:bg-black/5">
                        <div
                          className={cn(
                            "flex h-5 w-5 shrink-0 items-center justify-center rounded border-2 transition-colors",
                            tempBenifitId === b.id ? "border-saffron bg-saffron" : "border-black/20"
                          )}
                        >
                          {tempBenifitId === b.id && <Check className="h-3.5 w-3.5 stroke-[3] text-white" />}
                        </div>
                        <input
                          type="checkbox"
                          className="sr-only"
                          checked={tempBenifitId === b.id}
                          onChange={(e) => setTempBenifitId(e.target.checked ? b.id : "")}
                        />
                        {b.imageUrl && (
                          <div className="relative h-8 w-8 shrink-0 overflow-hidden rounded-full border border-black/10 bg-white">
                            <Image src={b.imageUrl} alt={getBenifitLabel(b, selectedDbLanguage)} fill className="object-cover" />
                          </div>
                        )}
                        <span className="text-sm font-semibold text-text-primary">{getBenifitLabel(b, selectedDbLanguage)}</span>
                      </label>
                    ))}
                    {filteredBenefits.length === 0 && (
                      <p className="py-4 text-center text-sm font-semibold text-text-primary/50">No benefits found.</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="overflow-y-auto pr-2 scrollbar-thumb-saffron">
                  <div className="space-y-1">
                    {filteredTemples.map((t) => (
                      <label key={t.id} className="flex cursor-pointer items-center gap-4 rounded-md p-2 transition-colors hover:bg-black/5">
                        <div
                          className={cn(
                            "flex h-5 w-5 shrink-0 items-center justify-center rounded border-2 transition-colors",
                            tempTempleId === t.id ? "border-saffron bg-saffron" : "border-black/20"
                          )}
                        >
                          {tempTempleId === t.id && <Check className="h-3.5 w-3.5 stroke-[3] text-white" />}
                        </div>
                        <input
                          type="checkbox"
                          className="sr-only"
                          checked={tempTempleId === t.id}
                          onChange={(e) => setTempTempleId(e.target.checked ? t.id : "")}
                        />
                        {t.imageUrl && (
                          <div className="relative h-8 w-8 shrink-0 overflow-hidden rounded-full border border-black/10 bg-white">
                            <Image src={t.imageUrl} alt={getTempleLabel(t, selectedDbLanguage)} fill className="object-cover" />
                          </div>
                        )}
                        <span className="text-sm font-semibold text-text-primary">{getTempleLabel(t, selectedDbLanguage)}</span>
                      </label>
                    ))}
                    {filteredTemples.length === 0 && (
                      <p className="py-4 text-center text-sm font-semibold text-text-primary/50">No temples found.</p>
                    )}
                  </div>
                </div>
              </div>
