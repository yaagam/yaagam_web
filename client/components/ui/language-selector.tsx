...
export function LanguageSelector({
  ...
  const switchLanguage = (newLang: Language) => {
    ...
    const currentPrefix = languages.find(
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            ...
            className={cn(
              "absolute right-0 top-[calc(100%+0.5rem)] z-60 w-50 rounded-lg border border-black/10 bg-white p-2 text-text-primary shadow-2xl shadow-black/20",
              menuClassName,
            )}
          >
            ...
            <div className="mb-2 flex items-center justify-between gap-3 rounded-lg bg-black/4 px-2 py-1.5">
              <span className="text-[11px] font-semibold uppercase tracking-wide text-text-primary/45">
                Text size
              </span>
              ...
              {languages.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex min-h-9 w-full items-center gap-3 rounded-md px-2.5 py-1.5 text-left text-sm font-semibold leading-5 transition-colors hover:bg-saffron/10 hover:text-saffron",
                      selected &&
                        "bg-saffron/15 text-[#6f3a11] hover:bg-saffron/20",
                    )}
                  >
                    <span className="min-w-0 flex-1 text-wrap-safe">
                      {languageNames[item]}
                    </span>
                    <span className="rounded bg-black/7 px-1.5 py-0.5 text-[11px] font-semibold leading-4 text-text-primary/35">
                      {languageCodes[item]}
                    </span>
                    {selected && (
