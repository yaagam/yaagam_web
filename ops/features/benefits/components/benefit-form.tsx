...
export function BenefitForm() {
  ...
  const save = useMutation({ mutationFn: (values: Values) => upsertBenefit([{ language: "EN", ...values.english }, ...targetLanguages.map((language) => ({ language, ...values.translations[language] }))], id), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["benefits"] }); await queryClient.invalidateQueries({ queryKey: ["benefit", id] }); success(isEdit ? "Benefit updated successfully." : "Benefit created successfully."); router.replace("/benefits"); }, onError: (error) => setFormError(errorMessage(error) ?? "Unable to save benefit.") });
