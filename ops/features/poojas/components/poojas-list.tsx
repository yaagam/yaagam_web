      ...
      <CardContent className="overflow-x-auto p-0">
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-border bg-muted text-xs uppercase text-muted-foreground">
            <tr><th className="px-5 py-3">Name</th><th className="px-5 py-3">Temple</th><th className="px-5 py-3">Price</th><th className="px-5 py-3">Weekly</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Actions</th></tr>
          </thead>
          <tbody className="divide-y divide-border">
            {isLoading && <tr><td className="px-5 py-8" colSpan={6}>Loading poojas</td></tr>}
            {data?.items.map((pooja) => (
              <tr key={pooja.id}>
                <td className="px-5 py-4 font-semibold">{pooja.name}</td>
                <td className="px-5 py-4">{pooja.templeName}</td>
                <td className="px-5 py-4">{formatCurrency(pooja.price)}</td>
                <td className="px-5 py-4">{pooja.isWeekly ? "Yes" : "No"}</td>
                <td className="px-5 py-4">{pooja.status}</td>
            ...
            {!isLoading && data?.items.length === 0 && <tr><td className="px-5 py-8 text-muted-foreground" colSpan={6}>No poojas found.</td></tr>}
          </tbody>
        </table>
      </CardContent>
