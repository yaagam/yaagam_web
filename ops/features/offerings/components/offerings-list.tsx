    ...
    <Card>
      ...
      <CardContent className="overflow-x-auto p-0">
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-border bg-muted text-xs uppercase text-muted-foreground"><tr><th className="px-5 py-3">Image</th><th className="px-5 py-3">Name</th><th className="px-5 py-3">Temple Amount</th><th className="px-5 py-3">Customer Base</th><th className="px-5 py-3">Selling Price</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Actions</th></tr></thead>
          <tbody className="divide-y divide-border">
            {isLoading && <tr><td className="px-5 py-8" colSpan={7}>Loading offerings</td></tr>}
            {data?.items.map((offering) => <tr key={offering.id}><td className="px-5 py-3">{offering.imageUrl ? <div className="relative h-12 w-16 overflow-hidden rounded-md border border-border"><Image src={offering.imageUrl} alt={offering.name} fill unoptimized className="object-cover" /></div> : <div className="h-12 w-16 rounded-md bg-muted" />}</td><td className="px-5 py-4 font-semibold">{offering.name}</td><td className="px-5 py-4">{formatCurrency(offering.templeAmount)}</td><td className="px-5 py-4">{formatCurrency(offering.basePrice)}</td><td className="px-5 py-4">{formatCurrency(offering.sellingPrice)}</td><td className="px-5 py-4">{offering.isActive ? "ACTIVE" : "INACTIVE"}</td><td className="px-5 py-4"><div className="flex justify-end gap-2"><Button asChild variant="outline" size="sm"><Link href={`/offerings/${offering.id}`}><Pencil className="h-4 w-4" />Edit</Link></Button><Button type="button" variant="destructive" size="sm" disabled={offering.poojaCount > 0} title={offering.poojaCount > 0 ? "This offering is used by one or more poojas" : undefined} onClick={() => setSelectedOffering(offering)}><Trash2 className="h-4 w-4" />Delete</Button></div></td></tr>)}
            {!isLoading && data?.items.length === 0 && <tr><td className="px-5 py-8 text-muted-foreground" colSpan={7}>No offerings found.</td></tr>}
          </tbody>
        </table>
